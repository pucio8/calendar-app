...
  const startDay = (date.getDay() + 6) % 7; // 0 = Poniedziałek (Monday)
  ...
  // ========================= POPRAWIONA SEKCJA START =========================
  // Pobierz dzisiejszą datę i zresetuj jej czas do północy dla precyzyjnych porównań.
  // Get today's date and reset its time to midnight for accurate comparisons.
  const today = new Date();
  today.setHours(0, 0, 0, 0);
  // ========================= POPRAWIONA SEKCJA KONIEC =========================

  // Adds empty cells at the beginning of the month
...
    const currentDate = new Date(year, month, i);
    ...
    if (currentDate < today) {
        ...
        // Logika obwódki dla dzisiejszego dnia
...
  const previousEventType = selectedEvents.get(dateString);
  ...
  // Zawsze usuwamy poprzednią klasę zaznaczenia, jeśli istniała.
  // Always remove the previous selection class if it existed.
  if (wasSelected) {
    dayDiv.classList.remove(previousEventType);
  }

  // Sprawdzamy, czy odznaczamy dzień (klikając ten sam typ ponownie).
  ...
  if (wasSelected && previousEventType === currentEventType) {
    ...
    // To jest nowe zaznaczenie lub zmiana typu.
